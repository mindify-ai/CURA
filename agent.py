...
def do_prediction(data):
    with RepoVM(image_name='swe_img:latest', repo_name=data['repo'], commit_hash=data['base_commit']) as vm:
        ...
        final_state = agent.invoke(
            input={
                'messages': [
                    ('system', 'You are an autonomous programmer, and you are going to propose a pull request based on the problem below. You are given several tools to help you with the task. The requested repo has been cloned to the root directory. You can use the tools to view, edit, and create files, and submit the patch.'),
